...
| Nov 10 – 20 2025 | 🤖 LLM Agents — External APIs & RAG Automation Framework
 | Built a multi-tool agentic AI system that connects LLMs to real-world APIs like web search, Gmail, Google Sheets, and RAG document memory. The agent autonomously perceives → plans → acts → remembers, executes tools via MCP (stdio + SSE), parses webpages/PDFs, and uses FAISS long-term memory for contextual recall. Designed as a fully modular, extensible framework for building real autonomous agents. | ⚙️ FastAPI · Gemini 2.0 Flash · Ollama · MCP (stdio + SSE) · FAISS · LlamaIndex · MarkItDown · Trafilatura · PyMuPDF4LLM · Google APIs (Sheets/Gmail/Drive) |
