...
| **Nov 19 – 20 2025** | 🤖 [**LLM Agents — External APIs & RAG Automation Framework**](https://github.com/sushant097/LLM-Agents-External-APIs) | Built a **modular agentic AI system** that connects LLMs to real-world tools: **web search, Gmail, Google Sheets, Drive, and local RAG memory**. The agent autonomously **perceives → plans → acts → remembers**, running tools through **MCP (stdio + SSE)**, parsing webpages/PDFs, and storing long-term context via **FAISS**. Fully extensible design for creating true autonomous AI agents. | ⚙️ *FastAPI · Gemini 2.0 Flash · Ollama · MCP (stdio + SSE) · FAISS · LlamaIndex · MarkItDown · Trafilatura · PyMuPDF4LLM · Google APIs (Sheets/Gmail/Drive)* |
| **Nov 20 – 28 2025** | 🤖 [**LLM Agents — External APIs & RAG Automation Framework**](https://github.com/sushant097/LLM-Agents-External-APIs) | Built an **end-to-end agentic system** where a single query from **CLI or Telegram** can search the web, extract tables from live pages/PDFs, create a **Google Sheet**, and **email the link** back to the user. The agent runs a full **Perceive → Plan → Act → Remember** loop over **MCP servers** (web, docs, GSuite, math, Telegram), with a modular core that’s easy to extend into richer autonomous workflows. | ⚙️ *Python · Gemini 2.0 Flash · MCP (stdio) · Google APIs (Sheets/Gmail) · Telegram Bot API · MarkItDown · Trafilatura · PyMuPDF4LLM · uv/CLI Orchestration* |
| **Nov 29 – Dec 5 2025** | 🧪 [**Hybrid-Agent-Planner — Semantic Cache + Heuristic Planning Agent**](https://github.com/sushant097/Hybrid-Agent-Planner) | Implemented a **hybrid planning agent** where an LLM writes Python `solve()` plans that run in a **sandboxed tool layer**, guarded by heuristics for safety, length, and numeric sanity. Added a global **historical conversation index + semantic cache** (Jaccard over keywords) so repeated or paraphrased queries instantly return cached `FINAL_ANSWER`s, while the planner still uses past examples to make smarter, cheaper multi-step tool calls. | ⚙️ *Python 3 · Gemini 2.0 Flash · FastMCP · MultiMCP Dispatcher · Semantic Cache over JSON Memory · Heuristic Planning Engine · CLI Agent Runner* |
