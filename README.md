...
| **Nov 29 – Dec 05 2025** | 🧩 [**Hybrid-Agent-Planner**](https://github.com/sushant097/Hybrid-Agent-Planner) | A high-reliability agent prioritizing **correctness and cost-efficiency**. Implements a **Hybrid Planning engine** that fuses LLM reasoning with **heuristic guardrails** and **secure Python sandboxing**. Features a **semantic cache (Jaccard similarity)** to instantly recall historical answers, bypassing the planning loop for recurring queries to save tokens. | ⚙️ *Python · Gemini 2.0 / Ollama · MultiMCP Dispatcher · FastMCP · Heuristic Planning Engine · Python Runtime Sandbox (exec + ModuleType) · Jaccard Similarity (Cache) ·JSON File Store* |
